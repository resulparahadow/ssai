...
async function load() {
    ...
    try {
        links.value = (
            await ofModel.smartLinks(props.modelId, { limit: 50 })
        ).links;
    } catch (e) {
        error.value =
            e instanceof Error ? e.message : 'Failed to load smart links.';
    } finally {
        loading.value = false;
    }
}
...
async function create() {
    ...
    try {
        await ofModel.createSmartLink(props.modelId, {
            ...
            free_trial_days:
                cType.value === 'free_trial' ? cDays.value : undefined,
        });
        cName.value = '';
        ...
        error.value =
            e instanceof Error ? e.message : 'Failed to create smart link.';
    } finally {
...
async function remove(id: string | null) {
    ...
    try {
        ...
        error.value =
            e instanceof Error ? e.message : 'Failed to delete smart link.';
    }
}
...
async function openDetailTab(id: string, tab: DetailTab) {
    ...
    try {
        if (tab === 'stats') {
            ...
            d.fans = await ofModel.smartLinkFans(props.modelId, id, {
                limit: 20,
            });
        } else if (tab === 'spenders') {
            d.spenders = (
                await ofModel.smartLinkSpenders(props.modelId, id, {
                    limit: 20,
                })
            ).rows;
        } else if (tab === 'clicks') {
            d.clicks = await ofModel.smartLinkClicks(props.modelId, id, {
                limit: 20,
            });
        } else if (tab === 'conversions') {
            d.conversions = await ofModel.smartLinkConversions(
                props.modelId,
                id,
                { limit: 20 },
            );
...
async function removeTag(id: string, tag: string) {
    ...
    try {
        d.tags = (
            await ofModel.removeSmartLinkTags(props.modelId, id, [tag])
        ).tags;
    } catch (e) {
    ...
    <div class="space-y-3">
        <div class="flex items-center justify-between gap-2">
            <p class="text-[12px] text-ss-text-3">
                Smart links auto-route fans and attribute revenue, clicks and
        ...
        >
            ...
            >
                <div class="flex items-center gap-3 p-3">
                    <button
                        type="button"
                        class="grid h-7 w-7 shrink-0 place-items-center rounded-lg text-ss-text-3 transition-transform hover:bg-ss-surface-2 hover:text-ss-text"
                        :class="{ 'rotate-180': expanded === l.id }"
                        @click="toggle(l.id)"
                    >
                        <ChevronDown :size="16" />
                    </button>
                    <div class="min-w-0 flex-1">
                        ...
                        >
                            ...
                            <span
                                ...
                                >{{ typeLabel(l.linkType) }}</span
                            >
                            <span
                                v-if="l.freeTrialDays"
                                class="rounded bg-ss-surface-2 px-1.5 py-0.5 text-[10px] text-ss-text-3"
                                >{{ l.freeTrialDays }}d free</span
                            >
                        </div>
                        <button
                            type="button"
                    ...
                    <div
                        class="hidden shrink-0 text-right text-[11px] text-ss-text-3 sm:block"
                    >
                        <div class="text-[13px] font-semibold text-ss-pos">
                            {{ money(l.revenue) }}
                        </div>
                        {{ l.clicks }} clicks · {{ l.conversions }} conv ·
                        {{ l.subscribers }} subs
                    </div>
                    <button
                        type="button"
                ...
                >
                    ...
                    <template v-else>
                        ...
                        >
                            <div
                                v-for="(val, lbl) in {
                                    Clicks: detail[l.id].stats!.clicks,
                                    Subs: detail[l.id].stats!.subs,
                                    Spenders: detail[l.id].stats!.spenders,
                                    Revenue: money(detail[l.id].stats!.revenue),
                                }"
                                :key="lbl"
                                class="rounded-lg bg-ss-surface p-2.5"
                            >
                                <div
                                    class="text-[14px] font-semibold text-ss-text"
                                >
                                    {{ val }}
                                </div>
                        ...
                        <div
                            v-else-if="
                                detail[l.id].tab === 'conversions' &&
                                detail[l.id].conversions
                            "
                            class="space-y-2"
                        >
                            <div class="grid grid-cols-3 gap-2 text-center">
                                <div
                                    v-for="(val, lbl) in {
                                        Conversions:
                                            detail[l.id].conversions!.summary
                                                .conversionsTotal,
                                        Subscribers:
                                            detail[l.id].conversions!.summary
                                                .subscribersTotal,
                            ...
                            <div
                                v-if="detail[l.id].conversions!.rows.length"
                                class="divide-y divide-ss-border overflow-hidden rounded-lg bg-ss-surface"
                            >
                                <div
                                    v-for="c in detail[l.id].conversions!.rows"
                                    :key="c.id ?? ''"
                                    class="flex items-center gap-2 p-2.5 text-[12px]"
                                >
                                    <span
                                        class="min-w-0 flex-1 truncate text-ss-text"
                                        >{{
                                            c.fan.name ||
                                            c.fan.username ||
                                            'Fan'
                                    ...
                                    <span
                                        class="rounded bg-ss-surface-2 px-1.5 py-0.5 text-[10px] text-ss-text-3"
                                        >{{ c.conversionType }}</span
                                    >
                                    <span class="text-ss-pos">{{
                                        money(c.amountNet)
                                    }}</span>
                                    <span class="text-ss-text-3">{{
                                        fmtDate(c.conversionAt)
                                    }}</span>
                                </div>
                            </div>
                            <p
                        ...
                        <div
                            v-else-if="
                                detail[l.id].tab === 'fans' && detail[l.id].fans
                            "
                            class="space-y-2"
                        >
                            <div class="grid grid-cols-3 gap-2 text-center">
                                <div
                                    v-for="(val, lbl) in {
                                        Fans: detail[l.id].fans!.summary
                                            .fansTotal,
                                        'Net revenue': money(
                                            detail[l.id].fans!.summary
                                                .revenueNetTotal,
                                        ),
                                        'Net tips': money(
                                            detail[l.id].fans!.summary
                            ...
                            <div
                                v-if="detail[l.id].fans!.rows.length"
                                class="divide-y divide-ss-border overflow-hidden rounded-lg bg-ss-surface"
                            >
                                <div
                                    v-for="f in detail[l.id].fans!.rows"
                                    :key="f.fanId ?? f.onlyfansId ?? ''"
                                    class="flex items-center gap-2 p-2.5 text-[12px]"
                                >
                                    <span
                                        class="min-w-0 flex-1 truncate text-ss-text"
                                        >{{
                                            f.name || f.username || 'Fan'
                                        }}</span
                                    >
                                    <span class="text-ss-text-3"
                                        >{{ f.messagesSentByFan }} msgs</span
                                    >
                                    <span class="text-ss-pos">{{
                                        money(f.revenueNet)
                                    }}</span>
                        ...
                        <div v-else-if="detail[l.id].tab === 'spenders'">
                            <div
                                v-if="detail[l.id].spenders!.length"
                                class="divide-y divide-ss-border overflow-hidden rounded-lg bg-ss-surface"
                            >
                                <div
                                    v-for="(s, i) in detail[l.id].spenders"
                                    :key="s.onlyfansId ?? i"
                                    class="flex items-center gap-2 p-2.5 text-[12px]"
                                >
                                    <span class="w-5 text-ss-text-3">{{
                                        i + 1
                                    }}</span>
                                    <span
                                        class="min-w-0 flex-1 truncate text-ss-text"
                                        >{{ s.username || s.onlyfansId }}</span
                                    >
                                    <span class="font-semibold text-ss-pos">{{
                                        money(s.revenue)
                                    }}</span>
                                </div>
                            </div>
                        ...
                        <div
                            v-else-if="
                                detail[l.id].tab === 'clicks' &&
                                detail[l.id].clicks
                            "
                            class="space-y-2"
                        >
                            <p class="text-[12px] text-ss-text-3">
                                <span class="font-semibold text-ss-text">{{
                                    detail[l.id].clicks!.total
                                }}</span>
                                total clicks
                            </p>
                            <div
                                v-if="detail[l.id].clicks!.rows.length"
                                class="divide-y divide-ss-border overflow-hidden rounded-lg bg-ss-surface"
                            >
                                <div
                                    v-for="c in detail[l.id].clicks!.rows"
                                    :key="c.id ?? ''"
                                    class="flex items-center gap-2 p-2.5 text-[12px]"
                                >
                                    <span
                                        class="min-w-0 flex-1 truncate text-ss-text-2"
                                        >{{ c.referrer || 'Direct' }}</span
                                    >
                                    <span
                                        v-if="c.countryCode"
                                        class="rounded bg-ss-surface-2 px-1.5 py-0.5 text-[10px] text-ss-text-3"
                                    ...
                                    <span
                                        v-if="c.isBot"
                                        class="rounded bg-ss-warn/15 px-1.5 py-0.5 text-[10px] text-ss-warn"
                                        >bot</span
                                    >
                                    <span class="text-ss-text-3">{{
                                        fmtDate(c.createdAt)
                        ...
                        <!-- Tags -->
                        <div
                            v-else-if="detail[l.id].tab === 'tags'"
                            class="space-y-2.5"
                        >
                            <div class="flex flex-wrap gap-1.5">
                                <span
                                    v-for="t in detail[l.id].tags"
                                    :key="t"
                                ...
                                >
                                    ...
                                    <button
                                        type="button"
                                        class="text-ss-text-3 hover:text-ss-neg disabled:opacity-50"
                                        :disabled="detail[l.id].tagBusy"
                                        @click="removeTag(l.id, t)"
                                    >
                                        &times;
                                    </button>
                                </span>
                                <span
                                    v-if="!detail[l.id].tags?.length"
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button
                                    type="button"
                                    class="rounded-lg bg-ss-accent px-3 py-1.5 text-[12px] font-semibold text-white disabled:opacity-50"
                                    :disabled="
                                        detail[l.id].tagBusy ||
                                        !detail[l.id].newTag.trim()
                                    "
                                    @click="addTag(l.id)"
                                >
                                    Add
